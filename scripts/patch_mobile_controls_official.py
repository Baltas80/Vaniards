...
GDevelop's official PlatformerMultitouchMapper. Jump uses the native platform
behavior action; attack and dash remain Vaniards FSM actions.
"""
...
def button_behavior(identifier):
    return {"name":"MultitouchButton","type":BUTTON,"ControllerIdentifier":1,
            "ButtonIdentifier":identifier,"TouchId":0,"TouchIndex":0,
            "IsReleased":False,"IsJustPressed":False,"Radius":0}

def mapper_behavior():
    return {"name":"PlatformerMultitouchMapper","type":MAPPER,
            "ControllerIdentifier":1,"JoystickIdentifier":"Primary"}

def make_joystick():
    transparent="assets/vaniards_touch_transparent.png"
    sprite={"hasCustomCollisionMask":False,"image":transparent,"points":[],
    ...
    animation={"name":"Idle","useMultipleDirections":False,
               "directions":[{"looping":False,"timeBetweenFrames":0.08,"sprites":[sprite]}]}
    child={"adaptCollisionMaskAutomatically":False,"updateIfNotVisible":False,"animations":[animation]}
    return {"assetStoreId":"","name":"MoveJoystick","type":JOYSTICK,"variant":"",
            "variables":[],"effects":[],"behaviors":[],
            "content":{"DeadZoneRadius":0.10,"ControllerIdentifier":1,"JoystickIdentifier":"Primary"},
            "childrenContent":{"Border":child,"Thumb":child.copy()}}

def instance(name,x,y,width=118,height=118,z=101):
    return {"angle":0,"customSize":True,"height":height,"keepRatio":True,"layer":"GUI",
            "locked":False,"name":name,"persistentUuid":str(uuid.uuid4()),"width":width,"x":x,"y":y,
            "zOrder":z,"numberProperties":[],"stringProperties":[],"initialVariables":[]}

def standard(conditions=None,actions=None):
    return {"type":"BuiltinCommonInstructions::Standard","conditions":conditions or [],
            "actions":actions or [],"events":[]}

def action(value,params):
    return {"type":{"value":value},"parameters":params}

def group(name,events):
    return {"colorB":74,"colorG":176,"colorR":228,"creationTime":0,"name":name,"source":"",
            "type":"BuiltinCommonInstructions::Group","events":events,"parameters":[]}

def patch(project_path:Path,reference_path:Path):
    data=json.loads(project_path.read_text(encoding="utf-8"))
    ref=json.loads(reference_path.read_text(encoding="utf-8"))
    extension=next((e for e in ref.get("eventsFunctionsExtensions",[]) if e.get("name")=="SpriteMultitouchJoystick"),None)
    if extension is None: raise RuntimeError("Official SpriteMultitouchJoystick extension not found")
    data["eventsFunctionsExtensions"]=[e for e in data.get("eventsFunctionsExtensions",[]) if e.get("name")!="SpriteMultitouchJoystick"]+[extension]

    stage=next((s for s in data.get("layouts",[]) if s.get("name")=="Stage"),None)
    if stage is None: raise RuntimeError("Stage layout not found")
    ...
    for name,identifier in {"TouchJump":"A","TouchAttack":"B","TouchDash":"C"}.items():
        ...
        if obj is None: raise RuntimeError(f"Missing object: {name}")
    ...
    if hero is None: raise RuntimeError("HeroHitbox not found")
    ...
    if not any(r.get("file")==transparent for r in resources):
        resources.append({"file":transparent,"kind":"image","metadata":"","name":Path(transparent).name,"smoothed":False,"userAdded":True})
    ...
    positions={"TouchJump":("SceneWindowWidth() - 330","SceneWindowHeight() - 120"),
    ...
    for name,(x,y) in positions.items():
        layout_events.append(standard([], [action("SetX",[name,"","=",x]),action("SetY",[name,"","=",y])]))
    layout_events.append(standard([], [action("SetX",["MoveJoystick","","=","120"]),action("SetY",["MoveJoystick","","=","SceneWindowHeight() - 120"])]))
    ...
    # Apply the action-button bridge after the official mapper is installed.
    scripts_dir=Path(__file__).resolve().parent
    sys.path.insert(0,str(scripts_dir))
    from patch_mobile_action_buttons import patch as patch_action_buttons
    patch_action_buttons(project_path)
    print("Official PlatformerMultitouchMapper installed; native jump and independent action buttons bridged.")

if __name__=="__main__":
    if len(sys.argv)!=3: raise SystemExit("usage: patch_mobile_controls_official.py <project.json> <reference-project.json>")
